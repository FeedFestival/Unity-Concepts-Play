...
public class BezierPointEditor : Editor {
	...
	void OnSceneGUI()
	{
		...
		var fmh_96_64_638246776533274347 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position)*0.2f, Vector3.zero, Handles.CubeHandleCap);
	...
	private static void HandleConnected(BezierPoint p){
		...
		var fmh_111_64_638246776533309080 = p.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1)*0.15f, Vector3.zero, Handles.SphereHandleCap);
		...
		var fmh_119_64_638246776533313032 = p.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2)*0.15f, Vector3.zero, Handles.SphereHandleCap);
	...
	private static void HandleBroken(BezierPoint p){
		...
		var fmh_131_64_638246776533317669 = Quaternion.identity; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1)*0.15f, Vector3.zero, Handles.SphereHandleCap);
		var fmh_132_64_638246776533321175 = Quaternion.identity; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2)*0.15f, Vector3.zero, Handles.SphereHandleCap);
